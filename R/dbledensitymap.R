dbledensitymap <- function(sp.obj, names.var, kernel = "triweight", names.attr = names(sp.obj), 
                           criteria = NULL, carte = NULL, identify = FALSE, cex.lab = 0.8, pch = 16,
                           col = c("grey", "lightblue3"), xlab = c("", ""), ylab = "", 
  ...
  pointfunc <- function() {
    ...
    while(!quit) {
      ...
      title("ACTIVE DEVICE", cex.main = 0.8, font.main = 3, col.main = "red")
      title(sub = "To stop selection, click on the right button of the mouse and stop (for MAC, ESC)", 
            cex.sub = 0.8, font.sub = 3, col.sub = "red")
      ...
      if (length(var1[obs]) > 1) {
        graphique(var1 = var1, obs = obs, alpha1 = alpha11, num = num_graph, 
                  ...
                  symbol = pch, kernel = kernel)
        
        graphique(var1 = var2, obs = obs, alpha1 = alpha21, num = num_graph_2, 
                  graph = graph2, labvar = labvar2, couleurs = col[2], symbol = pch,
                  kernel = kernel)
      } else {
        dev.set(num_graph)
        title(sub = "You have to choose at least two sites to represent the sub-density", 
  ...
  ####################################################
  # selection d'un intervalle sous la courbe de densite
  ####################################################
  ...
  inter1func <- function() {
    ...
    while (length(polyX) < 2) {
      dev.set(num_graph)
      title("ACTIVE DEVICE", cex.main = 0.8, font.main = 3, col.main = "red")
      title(sub = "Click two times to select an interval", 
            cex.sub = 0.8, font.sub = 3, col.sub = "red")
    ...
    obs <<- selectstat(var1 = var1, obs = obs, 
                       Xpoly = polyX[1], Ypoly = polyX[2], method = "Densityplot")
  ...
  inter2func <- function() {
    ...
    while (length(polyX) < 2) {
      dev.set(num_graph_2)
      title("ACTIVE DEVICE", cex.main = 0.8, font.main = 3, col.main = "red")
      title(sub = "Click two times to select an interval", cex.sub = 0.8, font.sub = 3,
            col.sub = "red")
      loc <- locator(1)
      polyX <- c(polyX, loc[1])
    }
    ...
    obs <<- selectstat(var1 = var2, obs = obs, 
  ...
  quitfunc2 <- function() {
    ...
    pdf(fig_save)
    if (graph1 == "Densityplot1")
      graphique(var1 = var1, obs = obs, alpha1 = alpha11, num = dev.list()[length(dev.list())], 
                graph = graph1, labvar = labvar1, couleurs = col[1], 
                symbol = pch, kernel = kernel)
    else
      graphique(var1 = var1, obs = obs, alpha1 = alpha11, num = dev.list()[length(dev.list())], 
    ...
    fig_save2 <- paste("fig_GeoXp", "_", k + 1, ".pdf", sep = "")
    pdf(fig_save2)
    if (graph2 == "Densityplot1")
      graphique(var1 = var2, obs = obs, alpha1 = alpha21, num = dev.list()[length(dev.list())], 
                graph = graph2, labvar = labvar2, couleurs = col[2], 
                symbol = pch, kernel = kernel)
    else
      graphique(var1 = var2, obs = obs, alpha1 = alpha21, num = dev.list()[length(dev.list())], 
