dblehistomap <- function(sp.obj, names.var, nbcol = c(10,10), type = c("count", "percent", "density"),
  ...
  bar1func <- function() {
    ...
    while (!quit) {
      ...
      obs <<- selectstat(var1 = var1, obs = obs, Xpoly = loc[1],
                         Ypoly = loc[2], method = "Histogram", nbcol = nbcol[1])
      
      graphique(var1 = var1, obs = obs, num = num_graph, graph = "Histogram",
                nbcol = nbcol[1], bin = type, labvar = labvar1, couleurs = col[1])
  ...
  bar2func <- function() {
    ...
    while (!quit)
    {
      ...
      if (is.null(loc)) {
        quit <- TRUE
        graphique(var1 = var2, obs = obs, num = num_graph_2, graph = "Histogram",
                  nbcol = nbcol[2], bin = type, labvar = labvar2, couleurs = col[2])
        next
      }
      obs <<- selectstat(var1 = var2, obs = obs, Xpoly = loc[1], 
                         Ypoly = loc[2], method = "Histogram", nbcol = nbcol[2])
  ...
  quitfunc2<-function() {
    ...
    pdf(fig_save)
    graphique(var1 = var1, obs = obs, num = dev.list()[length(dev.list())], 
              graph = "Histogram", nbcol = nbcol[1], 
              bin = type, labvar = labvar1, couleurs = col[1])
    dev.off()
    ...
    pdf(fig_save2)
    graphique(var1 = var2, obs = obs, num = dev.list()[length(dev.list())], 
              graph = "Histogram",nbcol = nbcol[2],
              bin = type, labvar = labvar2, couleurs = col[2])
    dev.off()
