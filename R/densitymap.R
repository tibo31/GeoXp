densitymap <- function(sp.obj, name.var, kernel = "triweight", names.attr = names(sp.obj), 
  ...
  if (interactive()) {
    if (!exists("GeoXp.open", envir = baseenv()) || length(ls(envir = .TkRoot$env, all.names = TRUE)) == 2) {
      ...
      if (get("GeoXp.open", envir = baseenv())) {
        stop("Warning : a GeoXp function is already open. Please, 
             close Tk window before calling a new GeoXp function to avoid conflict between graphics")
        } else {
          graphique(var1 = var, obs = obs, alpha1 = alpha1, num = num_graph, graph = graph, 
                    Xpoly = polyX2, labvar = labvar, symbol = pch, couleurs = col, kernel = kernel)
