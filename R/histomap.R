  ...
  barfunc <- function() {
    ...
    while (!quit) {
      ...
      if (is.null(loc)) {
        quit <- TRUE
        graphique(var1 = var, obs = obs, num = num_graph, graph = "Histogram", nbcol = nbcol, 
                  bin = type, labvar = labvar, symbol = pch, couleurs = col)
        next
      }
      obs <<- selectstat(var1 = var, obs = obs, Xpoly = loc[1], Ypoly = loc[2],
                         method = "Histogram", nbcol = nbcol)

      # graphiques
      graphique(var1 = var, obs = obs, num = num_graph, graph = "Histogram", 
                nbcol = nbcol, bin = type, labvar = labvar, symbol = pch, couleurs = col)
...
####################################################
  ...
  quitfunc2 <- function() {
    ...
    pdf(fig_save)
    graphique(var1 = var, obs = obs, num = dev.list()[length(dev.list())], graph = "Histogram", nbcol = nbcol,
              bin = type, labvar = labvar, couleurs = col, symbol = pch)
    dev.off()
