...
The first argument \code{sf.obj} is an sf object as defined by E. Pebezma
...
extracted from \code{sf.obj} by using the function \code{st\_coordinates} and \code{st\_point\_on\_surface}. \newline
...
the \code{sf.obj} to be used in the analysis.   \newline
...
principles remain the same.
 \begin{itemize}
  ...
  \item \code{carte:} in the case where \code{sf.obj} is a \code{SpatialPolygonDataFrame}, the user will have the
   opportunity to draw the polygons of Spatial unit by using the \code{Draw Spatial contours} button in the
   Tk window. However, if the \code{sf.obj} is a \code{POINT} spatial feature, the user may specify by using
   the \code{carte} option, another sf object containg another spatial layer. 
  \item \code{identify:} {if not NULL, the name of the variable for identifying observations on the map}.
...
histomap(immob.spdf, 7, nbcol = 15, type = "percent",
  criteria = criteria, carte = cont_midiP,
...
\begin{footnotesize}
<<echo=TRUE,eval=FALSE>>=
dbledensitymap(immob.sf, c("prix.vente", "prix.location"),
...
comes from the function \code{rq} included in the \pkg{quantreg} package.
...
\begin{footnotesize}
<<echo=TRUE,eval=FALSE>>=
neighbourmap(immob.df, "prix.vente", W.nb, identify = "Nom", cex.lab = 0.5,
  carte = midiP)
@
\end{footnotesize}
